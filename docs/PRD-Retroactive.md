...
- **Releases list (`/epics`)**: The epics table includes adjacent rollout date columns — **GTM Orgs** (planned GTM Access date from release/UI-rollout stages to **GTM Access and Prep**, or Aha *Phase 3: GTM Activation Cutoff*; column header shows the release-train planned GTM Access start from the **standard release schedule** when the release has any non–UI Framework epics, otherwise from the UI rollout timeline for UI-only releases; PMs with `launch.accessDates.update` set `actual_gtm_access_date` inline, may mark `gtm_access_na`, and confirm via `gtm_access_confirmed` checkbox — N/A and overridden dates styled distinctly), **Internal Orgs** (planned Internal Readiness start from release/UI-rollout stages or Aha *Phase 4b: Internal Readiness Distributed*; column header shows the release-train planned date using the same standard-vs-UI-only release rule as GTM Orgs; PMs set `actual_internal_readiness_date`, may mark `internal_readiness_na`, and confirm via `internal_readiness_confirmed` checkbox; N/A and overridden dates styled distinctly), **Cohort 1** (Dual Cohort only — hidden when Aha *Rollout Process* = Single GA; yellow highlight only when the date is an outlier — off-schedule release date or differs from the release train), and **GA** (yellow highlight only when the date is an outlier — off-schedule on Single GA, epic Scheduled GA differs from release train, or computed GA differs from train; `scheduled_ga_dev_date` when set, otherwise the release train’s Cohort 2 date from `release_schedule` — `cohort2_date` or the next **standard** release train’s `launch_date` (`Release YYYY.N` only; one-off releases such as “HRSG Competencies Sunset” are ignored) — with fallback to effective Cohort 1 + 28 calendar days; same rules as release status logic). **List fallbacks:** When an epic has no PM Cohort 1 date in Aha, the row uses the grouped release train’s `launch_date` for planned Internal / Cohort 1 / GA (Cohort 1 shown in italic with tooltip). Release stage config is loaded on the server with the page so GTM/Internal columns do not wait on a client fetch. Background cohort-2 backfill from Aha does not hide the table. The same effective Cohort 1 date and highlight apply on epic detail, home “My items”, success dashboards, and Slack notifications that reference cohort / launch timing.
...
- **Launch workback schedule states** (`context = 'launch'` only — release criteria have no derived window, only a manually entered Conditional Go due date): each artifact has a computed start (`tier_offset_days` T-minus GA) and completion date (its successor's start), and reads as `upcoming`, `in_window`, `late`, or `compressed`. **Compressed** covers the case where a release lands closer than the tier runway needs, so the artifact was due to start before the launch record existed (CLEARGO 2026.8) — the window never existed, so it is shown as at-risk rather than a missed deadline, and it does not block GO or trigger overdue nudges. Compression is **not permanent**: the artifact is re-granted the window it was designed to have (minimum 7 days) counted from the launch's creation date and capped at GA, and past that date it becomes `late` like anything else — blocking as a gate, escalating in notifications, and quoting the re-granted date rather than one that fell before the launch existed
- **Artifact links live on the Assets tab**: each of the five runway artifacts (Story Brief, Message Brief, Field Enablement Guide, Campaign Brief, Supporting Assets) carries its own Where-to-Find-It link, stored in launch_criterion_status.links. This replaced a pair of launch.brief_url / launch.feg_url columns surfaced on the Overview tab, which gave two of the five a URL and the other three nowhere to record one. The Supporting Assets table below it is Part 6 of the Campaign Brief template (11 rows, two optional), instantiated per launch from launch_asset_template
- **Launch status is derived, not stored** (`src/lib/launch-status.ts`, mirroring epic release status in `src/lib/epic-release-status.ts`): `launch.status` was a free-standing dropdown nobody kept current, so a launch that shipped in June still read "Planning" in August. The column now holds **only a manual override** and `NULL` means "derive it": **Planning** with no target date or before the tier's workback opens, **In Progress** inside the workback window (T-105 for TIER_1, T-77 for TIER_2 — the largest T-minus offset each tier's criteria carry), **Launched** on the target launch date, **Post-Launch** from the day after. Two values join the vocabulary that no date can produce, so they exist as overrides only: **On Hold** (paused) and **Cancelled** (abandoned). `archived` stays a separate flag — a cancelled launch is still listed until someone archives it. **Any** status can be pinned (a PMM can call a launch launched ahead of its date, or hold it In Progress past it) and the launch detail Status control offers an `Auto — <derived>` entry that clears the pin. Setting status needs `launches.manage` (PMM) **or** `launch.status.update` (Product Ops / CPO); every other launch field still needs `launches.manage` alone. A suspended launch stops generating work: On Hold and Cancelled launches are skipped by the launch artifact notification job and the Slack App Home, whose criteria keep their due dates and would otherwise nudge owners about a launch that is not happening. Migration `20260828000000_launch_status_override.sql` widens the CHECK constraint, drops NOT NULL / DEFAULT, and clears only those stored values that already agree with the dates — a genuine divergence is left pinned, since that is the only place a real decision could be hiding
- **Launch checklist aligned with the epic readiness surface** (`src/lib/launchChecklistFilters.ts`, `src/components/launch/LaunchCriterionDetailModal.tsx`): the two pages carry the same job but did not behave alike. The Checklist tab now has the epic tab's **My tasks / Overdue / Due soon** chips with the same combination rule (My tasks ANDs, the two date chips OR), and shows an `n of m shown` count plus a clear-filters empty state so a filtered launch is never mistaken for a finished one. Launch-specific semantics: **My tasks** matches the gate row owner **or any item inside it** (items are owned per function — the Beta gate alone spans PM, SE, UX, PMM and RevOps — so a row-only check hid most of a person's work); **Overdue** goes through `scheduleState`, which keeps a `compressed` row out of Overdue while its re-granted window is open; **Due soon** uses `effectiveDueDate` on a 14-day horizon. Each row also gains the epic row's chevron, opening **one detail panel** (accountable, note, where-to-find-it link, read-only item list) in place of the three separate modals the page opened from three different cells — the inline cells now open that same panel focused on their own field, mirroring `CommentsModal`'s `initialTab`, and all three fields go up in a single PATCH. Group headers on **both** surfaces now show `x/y done`, counting `GO`/`DONE` as done and leaving `NOT_APPLICABLE` out of the denominator so a fully handled group reads x/x. **Deliberately left divergent:** the status control — an epic criterion is a Go/No-Go judgment (traffic light, comment required on CONDITIONAL/NO_GO) while a launch row is work, and a launch gate's status already derives from its items (`gateStatusFromItems`); and the column labels, since epic `SOURCES` is Aha field provenance while launch `LINKS` is where the artifact lives, and epic `COMMENTS` is a thread that launch `NOTES` has no table to store. Threaded comments and attachments on launch rows remain unbuilt: `criterion_comment`/`criterion_attachment` carry a column named `launch_criterion_status_id` that actually FKs to `epic_criterion_status` (a leftover from the rename in migration 0018), so launch criteria have nowhere to put one
...
- **Launch artifact document setup (Netlify)**: Creating a launch instantiates a `launch_artifact` row per document its tier calls for and, where Google is configured, copies the template Doc into a per-launch Drive folder and stamps its header tokens. For a Tier 1/2 launch that is roughly twenty sequential Drive/Docs calls, which exceeds the **26s synchronous function cap**, so `POST /api/launches` hands the work to `/.netlify/functions/launch-artifacts-background` and answers **201** immediately with `artifacts_pending: true`; the artifacts panel polls the normal GET until every row carries a `doc_id`. `POST /api/launches/[id]/artifacts` with `action: 'ensure'` ("Create missing documents") uses the same worker and returns **202**, but only when there is real Google work to do — with nothing missing, or with no Google connection, it runs inline and returns the counts so the caller still gets "nothing missing" or the "connect Google" message. The handoff predicate is shared (`src/lib/artifacts/backgroundSetup.ts`) so both entry points cannot drift, and falls back to inline execution off-Netlify, where there is no cap. Requires env: `NETLIFY_ARTIFACT_DRAFT_SECRET` (falls back to `NETLIFY_HEART_SETUP_SECRET`), `NETLIFY_URL` (or `URL`). Idempotent throughout — `UNIQUE(launch_id, artifact_type)` plus a filename match against the Drive folder — so a missed or partial run is completed by the next press of the button, and a duplicate of a document someone is already editing is never created. The MCP `ensure-artifacts` and `draft-artifact` tools go through the same shared helpers (`startArtifactDraft`, `dispatchLaunchArtifactSetup`) rather than a second endpoint of their own; the earlier synchronous `/api/internal/artifacts` has been removed, along with `MCP_SECRET`.
...
- **Criteria Due Date Nudges**: Daily job (`/api/jobs/criteria-nudges`) sends Slack and email reminders for criteria based on due dates stored on `epic_criterion_status.condition_due_date` (recalculated from the release schedule anchor and rating timing as above):
  ...
  - **Conditional Go is not chased on the due-date schedule**: Scoring a criterion Conditional Go is a delivered verdict, not an unanswered question — the UI requires a comment for it exactly as it does for NO_GO, and readiness scoring already counts it as a real answer (0.5, capping a gating criterion's epic at 0.85). So the "1 week before / on due date / overdue" reminders do not apply to it. An unresolved condition still has to be settled before launch, so it instead re-surfaces **weekly once launch is within 14 days**, asking the owner to confirm the condition was met and convert to GO or NO_GO. Outside that window it stays silent in Slack/email while remaining visible on Home / My Items. Epics with no release or no launch date have nothing to anchor the confirmation to, so Conditional stays silent there too. See `isConditionalConfirmationDue()` in `src/lib/services/criteriaNotificationFilters.ts`
...
**Unified due-date definition (segment end + gate cascading):** Everywhere the product derives a criterion due date from launch stages (epic recalculation / `condition_due_date`, My Items on Home when unset, analytics on-time and PM-timeliness fallbacks), the base date is the **last day of that stage’s segment** on the timeline—the same rule as the Release Timeline chart—implemented in shared logic (`criterion-due-date.ts` + `releaseTimeline.ts`). Criteria whose `rating_timing` is a **gate stage** (`is_gate`, e.g. GTM Access and Prep, UX Preview) use cascading review windows before that segment end: **sub-criteria** (non-gate rows) are due **4 days** before the gate; **gate rollups** (`criterion.gate`) are due **1 day** before (calendar days on Release Schedule, business days on UI Rollout). Readiness criteria formerly tied to Internal Readiness are rated against **GTM Access and Prep** so inputs are ready before Go/No-Go; Internal Readiness remains a timeline phase and **Internal Orgs** operational tracking only.
...
An agenda-and-decision tracker for ClearCo's Packaging & Pricing Committee, built on release criteria data (spec: `docs/PaPriCo-report-spec.md`). Located at **Tools → PaPriCo Prep** in the main nav (`/paprico`); the old `/admin/settings/paprico` URLs redirect there.
...
- **Agenda generation (computed on read, no cron):** for the next open meeting, a release-derived item is auto-proposed when an epic has an enabled *gating criterion* (configured in `paprico_gating_criterion`, matched by `criterion_id`, never label) that is not complete (complete = `GO` or `NOT_APPLICABLE`; `CONDITIONAL_GO` stays open) and whose Ready-By stage date (`criterion.rating_timing` on the epic's launch timeline via `computeCriterionDueDateYmd`, with the category→stage fallback) falls within the lookahead (default 60 days, per-criterion override, default editable in `app_settings.paprico_default_lookahead_days`) **and the epic's tier is in that criterion's `tiers` scope** (default `TIER_1, TIER_2`, per-criterion, editable in the gating settings screen; epics with no tier are skipped). Tier scope is per criterion rather than one global floor: a broad criterion can watch Tier 3 while the SVP revenue-forecast review stays at Tier 1/2. Narrowing a scope does not retroactively close items already materialized. Generated items carry the criterion's `default_time_box_minutes` and an `owner_email` resolved as criterion decision owner (`epic_criterion_status.decision_owner_id`) → epic owner → null. Deferred items carry forward automatically; standing items persist until explicitly closed.
...
- **Gating scope (corrected 2026-09-11, migration `20260911000000_paprico_gating_scope_correction`):** the first live run generated 78 items across 48 epics and stopped reading as a packaging-and-pricing agenda. `Commercialization` (Customer Success & Ongoing Adoption, scoped to Tier 3) contributed 30 of them and was the *only* reason 19 epics appeared, and `Revenue Forecast & Risk Analysis` duplicated `Revenue forecast reviewed by SVP Sales + Head RevOps`. Both were disabled (`enabled = false`, not deleted, so tier scope / lookahead / time box survive a re-enable from the settings screen), and their already-materialized `proposed`/`on_agenda`/`deferred` items were closed — `decided` and `blocked` items were left alone. Result: 39 items across 28 epics, 25 of them packaging-and-pricing. Whether forecast review belongs on this agenda at all is a chair decision, left as a toggle rather than baked into the migration.
...
- **Time-box honesty:** the agenda reports `total_time_box_minutes` alongside `unbudgeted_item_count`. An item with no time box contributes zero minutes, so the total on its own reads as spare capacity when it is really an unknown; both numbers are rendered in the agenda header and in the Slack block.
- **Manual items can be linked:** `POST /api/paprico/items` and the `add-paprico-item` MCP tool accept optional `epic_id` + `criterion_id`, so a hand-added item is traceable back to the readiness matrix. `uq_paprico_item_open_release_pair` is scoped to `source = 'release'`, so duplicate suppression for linked standing items is enforced in application code instead: the create endpoints return 409 when the pair already has an open item, and the agenda sync skips materializing a pair any open item already claims. Linked standing items are *not* auto-closed when the criterion completes — a human wrote them, a human closes them.
- **One epic, one block:** items are generated per epic × gating criterion, so an epic with three open commercial criteria produced three top-level rows and read as three decisions. `groupAgendaItemsByEpic` (`src/lib/paprico/agenda.ts`) collapses a sorted section into one block per epic for both the agenda view and the Slack block. Grouping is presentational only — decisions attach to `paprico_item`, so items keep their identity, status, time box and drag-order inside the group, `PapricoAgenda` is unchanged, and existing `agenda_snapshot` rows render grouped without migration. Group order and band are inherited from each epic's most urgent item rather than re-sorted. An item with no epic (standing, orphan) is its own single-item group.
...
- **In-meeting mode:** `/paprico/meeting/[id]`, one item at a time, keyboard navigable (`j`/`k`/arrows, `d` for the decision form), saves immediately.
- **Calendar pre-fill:** the next event titled "PaPriCo" is fetched from the connected Google account's primary calendar on page load (`GET /api/paprico/next-calendar-meeting`, `src/lib/google/calendar.ts`). When no open meeting exists and none is created for that date, a "Start next meeting prep" banner at the top of the agenda creates the meeting in one click with the calendar's date and length; the New Meeting form also offers the same date/length pre-fill. Suggestions, not automation. Requires the `calendar.readonly` OAuth scope (reconnect Google in Settings → Integrations after the scope change) and the Calendar API enabled on the GCP project; degrades silently when absent (admins see the failure reason in the New Meeting form). Uses the shared OAuth connection only — the service-account fallback has no calendar.
...
- **Tables:** `paprico_meeting`, `paprico_item` (release-derived or standing; survives epic/criterion deletion as orphaned), `paprico_decision`, `paprico_gating_criterion` (`enabled`, `lookahead_days`, `tiers`, `default_time_box_minutes`; seeded by label match against release-context criteria: Packaging & Pricing Approved, Confirmed Pricing Communicated, Revenue forecast reviewed, Revenue Forecast & Risk Analysis, Commercialization, plus Product Name Confirmed / Unit Economics & Margin Floor Documented once they exist as release criteria — of which Revenue Forecast & Risk Analysis and Commercialization are seeded but disabled, see the gating-scope note above).
...
**Migrated from an external skill.** Revenue forecasting for ClearCo innovation products previously lived entirely in a separate `chrysalis-product-requirements` repo, run via a Claude Code skill (`/forecast`) that wrote markdown documents and pushed only summary ARR numbers + a static HTML link back into `epic_forecast_link`. It now lives in-app as a **Forecast tab** on the epic detail page: the model, its assumptions, and its narrative live natively in ClearGO, PMs edit assumptions interactively instead of regenerating a document externally, and a live multi-agent pipeline can generate a forecast for any epic from scratch. Only the pricing agent's live sheet connection (§12.4) still needs external setup before it's fully wired; everything else below is live. Retiring `forecasts/` from the Chrysalis repo (§12.6) is the last step, gated on this tab being verified for all 11 migrated products.
...
- **`forecast_assumptions`** — per-run assumption rows (`key`, `label`, bear/base/bull values, `confidence` = `confirmed` \| `hypothesis` \| `low_confidence`, `source_note` for provenance, `overridden_by`/`overridden_at` stamped when a PM edits a value — see §12.3a).
...
- **`forecast_generation_jobs`** — async job status table (`pending`/`running`/`completed`/`failed`), same shape as `heart_setup_jobs` (§5.7), backing the live generation pipeline (§12.5).
- **`epic_forecast_link`** (pre-existing, §—) gained a nullable `forecast_run_id` FK tying its shareable-link records to the canonical run. Superseded as the portfolio-rollup source by `forecast_runs` (§12.6a) but kept for its shareable HTML links.
...
Displays an epic's current forecast run: bear/base/bull `SegmentedControl`, an annual new-bookings table (cross-sell / net-new / total, with a 3-year total row) alongside a protected-ARR (churn reduction) column, quarterly detail when present, the assumptions table with confidence badges, and each narrative section. An on-demand "view original migrated document" panel fetches the archived raw markdown separately from the main payload (it can be tens of KB). Served by `GET /api/forecasts/[epicId]/current` (structured data, optional `?runId=` for a historical version) and `GET /api/forecasts/[epicId]/current/raw` (archived source), both following the existing `/api/forecasts/*` auth pattern (`X-ClearGo-Key` header or session auth).

**Monthly view + per-number calculation hovers (2026-09-11).** The period toggle gains a **Monthly** view (the engine has always stored month rows; they were fetched but not displayed), and every monthly number — cross-sell, net-new, total, protected ARR, in both the Monthly table and the constituent-months breakdown inside a quarter/year row's expanded explanation — gets a hover card (`MonthMathHover` in `ForecastPageContent.tsx`) built so anyone in the company can challenge the assumptions, not just read the result: (1) the arithmetic chain for that exact month (eligible pool × 3-year penetration → target accounts, × ramp fraction reached that month, × ACV ÷ 12, split cross-sell/net-new; the churn variant shows at-risk pool × protection rate × linear ramp), every derived figure reverse-derived from the row's own stored numbers so the hover can never contradict the table; (2) an inputs table with a plain-language explainer of what each input *is* (`MODEL_INPUT_DEFS`), its scenario value, its source note, and its confidence badge; (3) a pointer to Edit → Save New Version as the sanctioned way to record a disagreement. Monthly cells render exact dollars (`formatUsdExact`) rather than the floored-$K house format, which collapses sub-$1K months to "$0K". Migrated hand-built runs, whose inputs may not parse numerically, degrade to the inputs table plus a pointer at the archived original document. The expanded-row explanation panel's inputs table picked up the same explainer + source-note treatment.

#### 12.3a Editable assumptions and versioned saves
The 11 migrated forecasts are heterogeneous, hand-built models (named-account churn pools, two-independently-corrected-pool splits, run-rate models) — no single formula could mechanically recompute all of them from an edited assumption value. So **Edit** mode makes both assumption values and period figures directly editable; **Save New Version** (`POST /api/forecasts/[epicId]/versions`) inserts a new `forecast_runs` row (`source: generated`), demotes the prior current run's `is_current` flag (never deletes it), stamps `overridden_by`/`overridden_at` only on assumption values that actually changed, and carries narrative forward unedited. A version-history `Select` (`GET .../versions`) lets a PM browse prior runs read-only.

#### 12.4 Pricing
`src/lib/pricing/` drives ClearCo's live **"2026 Price Calculator"** Google Sheet (owned by the product owner, actively maintained) via the Sheets API against a ClearGo-owned duplicate — reading Google's own computed `Quote` tab output rather than reimplementing the sheet's formulas in code, deliberately avoiding a prior attempt (Customer Hub's `pricing_catalog`, since retired as a POC) that reimplemented pricing formulas and drifted from the live sheet. Intended as a stopgap until Salesforce CPQ replaces manual pricing entirely, behind a single interface (`getPackagePrice`, `getAddonPrice`) so swapping backends later is a one-module change. **Not usable yet** — needs a Google service account + a duplicated, shared sheet (env: `PRICING_GOOGLE_SERVICE_ACCOUNT_EMAIL`, `PRICING_GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`, `PRICING_SHEET_ID`) and the actual cell map (`CELL_MAP` in `src/lib/pricing/index.ts`, currently unfilled nulls rather than a guess) for the real duplicated sheet's input/output cells. `getPackagePrice` throws a clear error until both are set; the live generation pipeline (§12.5) catches that and falls back to a research-based pricing hypothesis.

#### 12.5 Live generation pipeline
"Generate Forecast" on the Forecast tab (`src/lib/forecast/orchestrator.ts`) runs Market/Competitive Research (`generateObject`) → the Pricing Agent (§12.4, with its fallback) → the deterministic engine (§12.5a) → a Narrative agent, then persists the result as a new current `forecast_runs` version via `src/lib/forecast/persist.ts` (shared with §12.3a's manual-edit endpoint). Backgrounded on Netlify production exactly like §5.7's HEART AI setup: `forecast_generation_jobs` row → `POST` to `netlify/functions/forecast-generation-background.ts` with a shared secret (`NETLIFY_FORECAST_GENERATION_SECRET`) → `202` + `job_id` → client polls `GET /api/forecasts/[epicId]/generate-status` every 2.5s (5-minute timeout). Runs synchronously in local dev (no 26s function limit there). Triggered via `POST /api/forecasts/[epicId]/generate`.

#### 12.5a Deterministic engine
`src/lib/forecast/engine.ts` (`computeForecastPeriods`, unit-tested in `__tests__/engine.test.ts`) is a pure-function ramp × price × volume model — eligible pool × 3-year penetration, ramped in against a named `RampProfile` (e.g. the AI Notetaker baseline S-curve), priced at ACV, split cross-sell vs. net-new — rolled up to month/quarter/year for all three scenarios. Churn reduction is modeled separately against a named at-risk ARR pool with its own linear protection ramp, never summed into bookings. This is what the live generation pipeline uses for a **new** forecast, where the model shape is uniform by design — unlike the 11 migrated historical forecasts, which stay directly editable per §12.3a rather than being forced through one formula.

#### 12.6 Portfolio rollup (`/portfolio/forecasts`)
`GET /api/forecasts/summary` merges each epic's current `forecast_runs` (base-scenario annual bookings + protected ARR, synthesized into the same shape the page already rendered, tagged `source: 'forecast_run'`) ahead of any legacy `epic_forecast_link` rows (`source: 'epic_forecast_link'`) for the same epic. This is what replaces `CONSOLIDATED.md` as the portfolio-wide rollup — live current-run data across all migrated and newly-generated epics, not a hand-maintained file in another repo.

#### 12.7 Retirement of the external skill (planned, not yet built)
...
- `actual_gtm_access_date` (Text, YYYY-MM-DD) - PM-entered actual/revised GTM org access date
- `gtm_access_confirmed` (Boolean) - Manual confirmation that GTM access has occurred
- `gtm_access_na` (Boolean) - PM marked GTM org access as not applicable for this epic
- `actual_internal_readiness_date` (Text, YYYY-MM-DD) - PM-entered actual/revised Internal Readiness distributed date
- `internal_readiness_confirmed` (Boolean) - Manual confirmation that Internal Readiness has been distributed
- `internal_readiness_na` (Boolean) - PM marked Internal Readiness as not applicable for this epic
...
- **forecast_assumptions**: per-run assumption rows (`key`, `label`, `value_bear`/`value_base`/`value_bull`, `confidence` ∈ confirmed | hypothesis | low_confidence, `source_note`, `overridden_by`/`overridden_at` stamped when a PM edits a value via the versioned-save endpoint).
...
- **forecast_generation_jobs**: backs the live generation pipeline (`POST /api/forecasts/[epicId]/generate`); same shape as `heart_setup_jobs` above.
...
3. System filters criteria by:
   ...
   - **Overdue back-off**: For the overdue window, suppresses rows nudged more recently than the ageing interval allows (daily ≤7 days overdue, weekly ≤30, fortnightly beyond) — see `isOverdueNudgeDue()` in `src/lib/services/criteriaNotificationFilters.ts`. Conditional Go rows are exempt; their cadence comes from the next rule.
   - **Conditional Go confirmation window**: Conditional Go rows are excluded from all due-date reminder windows and instead nudge weekly only while launch is 0–14 days away (`isConditionalConfirmationDue()`), asking the owner to confirm the condition and convert to GO/NO_GO. Silent when launch is further out, already passed, or unknown. `CONDITIONAL` remains "incomplete" for `isCriterionCompleteForNotifications()` and for the Home list — this rule changes the cadence, not the status semantics.
...
- **Authentication**: OAuth 2.1 bearer token issued by this app (below). A 401 carries `WWW-Authenticate: Bearer resource_metadata="…"`, which is what makes a client discover the flow. The legacy `X-ClearGo-Key` header is still accepted for the team-management tools, but authenticates a service account with **no roles**, so every capability-gated tool refuses it.
- **Tools — team management** (5): `list_team_members`, `get_1on1_prep`, `list_member_epics`, `list_member_blockers`, `get_epic_detail`. Direct reports resolve from the authenticated caller; this was previously a hardcoded manager address, so every other manager received somebody else's reports. snake_case names are kept deliberately — renaming would break the TTS client and existing Claude Desktop history.
- **Tools — PaPriCo Prep** (7): reads `list-paprico-meetings`, `get-paprico-agenda`, `list-paprico-decisions`, `list-paprico-items`, `get-next-paprico-meeting` (the Google Calendar hint behind the New Meeting form, degrading to `found:false` when Google is not connected or the `calendar.readonly` scope predates the connection); writes `create-paprico-meeting`, `add-paprico-item`, `publish-paprico-agenda`. Publishing freezes the computed agenda onto the meeting and is conditioned on the meeting still being a draft, so two concurrent publishes produce one snapshot and one refusal — shared with the HTTP route via `src/lib/paprico/publishAgenda.ts`.
- **Tools — forecasting** (4): `get-forecast`, `list-forecasts`, `generate-forecast`, `get-forecast-status`. Generation is gated on the new **`forecast.generate`** capability (PM, PRODUCT_OPS, CPO by default): it runs several AI agents, takes minutes and replaces the epic's current forecast. Note the HTTP route behind the Generate Forecast button checks only authentication, so the connector is deliberately the stricter of the two pending a decision on the route. The start/poll orchestration — the environment sniff, the job row, the background trigger and the rollback when the trigger fails — is extracted to `src/lib/forecast/startGeneration.ts`, mirroring `startArtifactDraft`, so both callers start runs the same way.
- **Tools — notifications** (1): `get-my-notifications` reads `notification_log` — what ClearGO has already told the caller, over which channel, and whether delivery failed. A reminder about something someone has been chased about four times is noise, and an undelivered nudge is indistinguishable from an ignored one unless something says so.
- **Shared with the in-app assistant**: the registry table is the single source for two transports. `src/lib/ai/mcpTools.ts` adapts every entry into a Vercel AI SDK tool for the ClearGO assistant, running as an actor whose roles are read from `app_user`, so capability gates behave exactly as they do over OAuth. Before this, the assistant had its own independently written tools: `update_criterion_status` wrote to `epic_criterion_status` directly with no `criteria.status.update` check (and `/api/chat` only verifies authentication), no readiness recompute, no `criterion_status_history` row and no gate sign-off nudge; `get_my_pending_actions` resolved ownership as `decision_owner_id` alone, so a PM owning items through the pod→PM mapping was told they were caught up. Both are deleted. Tests assert the parity property itself.
- **Tools — launch artifacts** (15): reads `list-launches`, `search-launches`, `get-launch`, `get-launch-context`, `list-artifacts`, `get-artifact`, `diff-artifact`, `explain-claim`, `artifact-chat`; writes `ensure-artifacts`, `draft-artifact`, `draft-section`, `update-artifact`, `answer-flags`, `review-artifact`. Writes are capability-checked against the caller's roles (`launchArtifact.draft` / `.review` / `.approve`), the same gates the UI route applies. Drafting returns immediately with `DRAFTING` and runs in the background function, because a synchronous Netlify function is capped at 26s; callers poll `get-artifact`.
- **Tools — readiness loop** (4): `get-my-work` (scoped by the OAuth subject, so it needs no arguments), `get-pending-gtm-access`, `get-epic-criteria`, and the write `update-criterion-status`. The write is a thin wrapper over `scoreEpicCriterion`, which owns the capability check (`criteria.status.update`), status normalisation, the Not Applicable rules, the audit and history rows, the gate sign-off nudge and the readiness recompute; the tool passes the service-role client through, because `recomputeEpicReadiness` otherwise defaults to the cookie-backed client and rescores against an empty criteria set.
- **Tools — navigation** (3): `find-epics`, `get-epic` (returns the *derived* release status the UI shows, not the stored column), `list-releases`
- **Tools — measurement** (4): `get-heart-metrics`, `get-success-metrics`, `get-success-scorecards`, `list-retros`. `get-success-metrics` reproduces a rule that lives only in the HTTP route: an unpublished plan is hidden unless the caller holds `settings.successMeasurement.update`, and the tool says the plan is hidden rather than returning an empty list.
- **Tools — Roadmap Rewind** (7): reads `list-roadmap-snapshots`, `get-roadmap-movements`, `get-roadmap-delivery-metrics`, `get-strategic-items`, `get-confidence-rating`; writes `adjust-confidence` (`roadmap.confidence.adjust`) and `set-impact-override` (`roadmap.impactOverride.write`). The confidence write's rating update and history append are shared with the HTTP route via `src/lib/roadmap/confidenceWrite.ts`.
- **Tools — reporting and meetings** (7): `get-analytics` (one tool with a report enum, gated on `analytics.read`), `list-paprico-meetings`, `get-paprico-agenda`, `list-paprico-decisions` (all `paprico.manage`), `get-forecast` (read only; keyed by Aha reference, not epic UUID), `get-epic-decisions`, `get-epic-story-brief` (the epic-scoped brief, distinct from the `story_brief` launch artifact).
- **Capability checks**: every gated tool resolves DB overrides via `actorCan` (`src/lib/permissions-server.ts`). Tools previously used `canRolesPerform`, which closes over `DEFAULT_RULES`, so an admin who narrowed a capability in Settings had narrowed it in the web app and not in the connector.
- **Rate limiting**: `POST /api/mcp` is throttled per authenticated caller (100/min), keyed on email rather than IP so an office behind one egress address does not share a bucket.
- **Not exposed**: forecast *generation* (its start/poll orchestration is inline in the route rather than extracted like `startArtifactDraft`), story-brief edits (version bump plus change-log append, inline), delegation and criterion comments (no extracted service), and the activity feed (397 lines of inline multi-table merge; its content is reachable via decisions, criteria and my-work).
- **Implementation**: `src/lib/mcp/server.ts` (factory, takes the authenticated actor), `src/lib/mcp/tools/` (tool modules + registry table), `src/lib/mcp/actor.ts` (OAuth subject to `app_user`, needed because attribution columns are uuid FKs and roles are re-read from the row rather than trusted from the hour-long token), `src/lib/mcp/queries.ts` (DB query logic shared with Netlify functions), `src/app/api/mcp/route.ts` (route handler, bearer verification, throttling)
- **Consumers**: Claude Desktop as a custom connector (per-user OAuth); Team Tactical Sync (TTS) via `StreamableHTTPClientTransport` for Daily Check-in brief generation

#### OAuth 2.1 Authorization Server
✅ **IMPLEMENTED** — ClearGO is its own authorization server so a teammate can add the MCP connector with nothing but a URL. Replaces a stdio MCP server that required the Supabase service-role key in a plaintext file on every laptop and enforced no authorization at all.

- **Discovery**: `/.well-known/oauth-protected-resource/api/mcp` (RFC 9728) and `/.well-known/oauth-authorization-server` (RFC 8414)
- **Endpoints**: `/api/oauth/register` (RFC 7591 dynamic client registration — what removes all manual configuration), `/api/oauth/authorize`, `/api/oauth/token`, `/api/oauth/revoke` (RFC 7009)
- **Consent**: `/oauth/consent` — a server-rendered page naming the client and the signed-in user. Registration is open by design, so this click is the actual authorization decision.
- **PKCE**: S256 required; `plain` is neither advertised nor accepted
- **Access tokens**: stateless JWTs signed with `MAGIC_LINK_SECRET`, 1h TTL, audience-bound to the MCP endpoint, carrying email + roles. A `typ` claim distinguishes them from magic-link tokens signed with the same key.
- **Refresh tokens**: rotating, stored only as SHA-256. Roles are re-read from `app_user` on every exchange, so reduced access takes effect within the hour rather than at the end of a 30-day refresh chain.
- **Replay handling**: a second presentation of an authorization code revokes every refresh token issued from it
- **Tables**: `mcp_oauth_client`, `mcp_oauth_authorization_code`, `mcp_oauth_token` (RLS on, no policies — service-role only)
- **Implementation**: `src/lib/oauth/` (config, store, tokens, pkce), `src/app/api/oauth/*`, `src/app/.well-known/*`
- **Documentation**: `docs/MCP-Connector.md`
...
3. **Criteria Due Date Nudges** — ✅ **IMPLEMENTED**: Daily job (`/api/jobs/criteria-nudges`) sends reminders for criteria based on due dates:
   - **Frequency**: 1 week before, on due date, then after overdue on an ageing back-off (daily for the first week, weekly to day 30, fortnightly after). Conditional Go is exempt and instead nudges weekly only within 14 days of launch, to confirm the condition was met
