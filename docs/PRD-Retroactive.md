...
- **Tools — team management** (5): `list_team_members`, `get_1on1_prep`, `list_member_epics`, `list_member_blockers`, `get_epic_detail`. Direct reports resolve from the authenticated caller; this was previously a hardcoded manager address, so every other manager received somebody else's reports.
