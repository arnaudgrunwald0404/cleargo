...
- **Tools — team management**: `list_team_members`, `get_1on1_prep`, `list_member_epics`, `list_member_blockers`, `get_epic_detail`
...
- **Implementation**: `src/lib/mcp/server.ts` (factory, takes the authenticated actor), `src/lib/mcp/tools/` (artifact tools + registry), `src/lib/mcp/queries.ts` (DB query logic shared with Netlify functions), `src/app/api/mcp/route.ts` (route handler + bearer verification)
