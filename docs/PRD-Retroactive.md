...
- **Calendar pre-fill:** the New Meeting form suggests the next event titled "PaPriCo" from the connected Google account's primary calendar (`GET /api/paprico/next-calendar-meeting`, `src/lib/google/calendar.ts`) — a one-click date/length pre-fill, not automation. Requires the `calendar.readonly` OAuth scope (reconnect Google in Settings → Integrations after the scope change) and the Calendar API enabled on the GCP project; degrades silently when absent. Uses the shared OAuth connection only — the service-account fallback has no calendar.
