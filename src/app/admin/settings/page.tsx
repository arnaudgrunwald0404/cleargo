...
export default function AdminSettingsPage() {
  ...
  const [launchReleases, setLaunchReleases] = useState<
    Array<{ releaseName: string; launchDate: string | null }>
  >([]);
  const [launchReleasesLoading, setLaunchReleasesLoading] = useState(false);

  // AHA fields state
  const [availableAhaFields, setAvailableAhaFields] = useState<
    Array<{ alias: string; label: string; key: string | null; type?: string }>
  >([]);
  ...
  const handleAddStage = async () => {
    ...
    try {
      const sortOrder =
        launchStages.length > 0 ? Math.max(...launchStages.map((s) => s.sort_order)) + 1 : 1;
  ...
  const fetchAhaFields = async () => {
    ...
    try {
      ...
      console.log(
        'Standard fields:',
        data.fields?.filter((f: any) => f.type === 'standard')
      );
  ...
  const fetchEmailTemplates = async () => {
    ...
    try {
      ...
      setEmailTemplates({
        ...
        update_criteria_subject:
          data.update_criteria_subject || DEFAULT_EMAIL_TEMPLATES.update_criteria_subject,
        update_criteria_html:
          data.update_criteria_html || DEFAULT_EMAIL_TEMPLATES.update_criteria_html,
      });
      return;
      // Default templates
  ...
  const autoSaveEmailTemplates = async () => {
    debugLog({
      location: 'page.tsx:autoSaveEmailTemplates',
      message: 'autoSaveEmailTemplates START',
      data: {
        invite_subject: emailTemplates.invite_subject?.substring(0, 50),
        invite_html_length: emailTemplates.invite_html?.length,
      },
      hypothesisId: 'B',
    });
    ...
    try {
      ...
      debugLog({
        location: 'page.tsx:autoSaveEmailTemplates',
        message: 'Calling patchEmailTemplates',
        data: {
          payloadKeys: Object.keys(payload),
          hasInviteSubject: !!payload.email_template_invite_subject,
        },
        hypothesisId: 'B,C',
      });
      ...
      debugLog({
        location: 'page.tsx:autoSaveEmailTemplates',
        message: 'patchEmailTemplates SUCCESS',
        data: {},
        hypothesisId: 'B',
      });
    } catch (error: any) {
      debugLog({
        location: 'page.tsx:autoSaveEmailTemplates',
        message: 'autoSaveEmailTemplates ERROR',
        data: { errorMessage: error?.message, errorName: error?.name, errorString: String(error) },
        hypothesisId: 'B,E',
      });
  ...
  const fetchPermissions = async () => {
    ...
    try {
      ...
      setRules(
        data.overrides && Object.keys(data.overrides).length > 0 ? data.overrides : data.rules || {}
      );
    } catch (e) {
  ...
  useEffect(() => {
    debugLog({
      ...
      data: {
        ...
        hasTemplates: !!emailTemplates,
        invite_subject_length: emailTemplates?.invite_subject?.length,
      },
      hypothesisId: 'A',
    });
    ...
    debugLog({
      location: 'page.tsx:useEffect',
      message: 'Scheduling auto-save',
      data: { willSaveIn: '2000ms' },
      hypothesisId: 'A,D',
    });
    ...
    emailTemplates.invite_subject,
    emailTemplates.invite_html,
    emailTemplates.remind_subject,
  ...
  const handleSynchronizeFields = async () => {
    ...
    try {
      ...
      if (result.errors && result.errors.length > 0) {
        setError(
          `Some launches failed to sync. ${result.synced} succeeded, ${result.failed} failed.`
        );
      }
    } catch (error: any) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-8">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {activeSection === 'criteria' && <CriteriaManager />}
