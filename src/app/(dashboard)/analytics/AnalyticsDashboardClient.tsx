...
import { PlanVsActualTab } from '@/components/analytics/PlanVsActualTab';
...
type ReleasesTabValue = 'launch-metrics' | 'timeliness' | 'usage' | 'success-overview';
type RoadmapTabValue = 'plan-vs-actual' | 'rewind';
type TabValue = ReleasesTabValue | RoadmapTabValue;

const RELEASES_TABS: ReleasesTabValue[] = ['launch-metrics', 'timeliness', 'usage', 'success-overview'];
const ROADMAP_TABS: RoadmapTabValue[] = ['plan-vs-actual', 'rewind'];

function isReleasesTab(t: TabValue): t is ReleasesTabValue {
  return (RELEASES_TABS as readonly string[]).includes(t);
}

function isRoadmapTab(t: TabValue): t is RoadmapTabValue {
  return (ROADMAP_TABS as readonly string[]).includes(t);
}
...
export default function AnalyticsDashboardClient() {
  ...
  const rewindFeatureOn = isEnabled(FEATURE_ROADMAP_REWIND, featureFlags);

  const [checkingAccess, setCheckingAccess] = useState(true);
  const [hasAccess, setHasAccess] = useState(false);
  const [userRoles, setUserRoles] = useState<string[]>([]);
  const [activeTab, setActiveTab] = useState<TabValue>('launch-metrics');
  const [loadedTabs, setLoadedTabs] = useState<Set<ReleasesTabValue>>(new Set());
  const loadedTabsRef = useRef<Set<ReleasesTabValue>>(new Set());
  ...
  useEffect(() => {
    ...
    const legacyRoadmapTab = searchParams.get('roadmapTab');

    let next: TabValue | null = null;

    if (
      tabParam === 'launch-metrics' ||
      tabParam === 'timeliness' ||
      tabParam === 'usage' ||
      tabParam === 'success-overview' ||
      tabParam === 'plan-vs-actual' ||
      tabParam === 'rewind'
    ) {
      next = tabParam;
    } else if (legacySection === 'roadmap') {
      next = legacyRoadmapTab === 'rewind' ? 'rewind' : 'plan-vs-actual';
    } else if (legacySection === 'releases') {
      next = 'launch-metrics';
    }

    if (next === 'rewind' && !rewindFeatureOn) {
      next = 'plan-vs-actual';
    }
    ...
    // Only react to URL changes; activeTab is intentionally excluded
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, rewindFeatureOn]);
  ...
  // Lazy load tab data when a releases tab is selected
  useEffect(() => {
    if (!hasAccess) return;
    if (!isReleasesTab(activeTab)) return;
  ...
  const headerSubtitle = isRoadmapTab(activeTab)
    ? 'Roadmap plan vs actual and movement analytics'
    : 'Adoption, Compliance, Timeliness, Outcomes';

  const showReleasesFilters = isReleasesTab(activeTab);
  ...
  return (
    ...
    >
      ...
      >
        <div className="mb-8">
          <Group justify="space-between" align="flex-start">
            ...
            <Button
              ...
              variant="light"
              loading={showReleasesFilters ? isLoading : false}
            >
              Refresh
            </Button>
          </Group>
        ...
        <Stack gap="md">
          <Tabs value={activeTab} onChange={(value) => handleTabChange((value || 'launch-metrics') as TabValue)}>
            <Tabs.List>
              ...
              <Tabs.Tab value="plan-vs-actual">Plan vs Actual</Tabs.Tab>
              {rewindFeatureOn ? (
                <Tabs.Tab value="rewind">Roadmap Rewind</Tabs.Tab>
              ) : null}
            </Tabs.List>

            {showReleasesFilters ? (
          <Group mt="md" mb="lg" align="flex-end" gap="sm">
        ...
        <Tabs.Panel value="plan-vs-actual" pt="md">
          <PlanVsActualTab userRoles={userRoles} />
        </Tabs.Panel>

        {rewindFeatureOn ? (
          <Tabs.Panel value="rewind" pt="md">
            <RoadmapRewindView />
          </Tabs.Panel>
        ) : null}
          </Tabs>
