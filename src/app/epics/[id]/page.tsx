...
export default function EpicDetailPage() {
  ...
  const [launchStages, setLaunchStages] = useState<
    Array<{ id: number; name: string; sort_order: number; duration_days: number | null }>
  >([]);
  ...
  async function loadData() {
    try {
      ...
      const applies = (
        app: 'ALL' | 'TIER_1_ONLY' | 'TIER_1_AND_2',
      ...
      const withApplicability = merged.map((item: any) => ({
        ...
        notRequired:
          ...
          (item?.criterion?.tier_applicability
            ? !applies(item.criterion.tier_applicability as any, data.tier as any)
            : false),
      }));

      // Sort by criterion sort_order
      const sorted = withApplicability.sort(
        (a: any, b: any) => (a.criterion?.sort_order || 0) - (b.criterion?.sort_order || 0)
      );
      ...
      const getReleaseName = (): string | null => {
        ...
        if (ahaFields.standard_fields && typeof ahaFields.standard_fields === 'object') {
          ...
          const releaseName =
            standardFields?.aha_release_name || standardFields?.release?.name || null;
      ...
      sorted.forEach((item: any) => {
        ...
        if (
          criterionEmail === "[name of pod's product manager]" ||
          (criterionEmail && criterionEmail.toLowerCase().includes('pod'))
        ) {
      ...
      if (approverEmails.size > 0) {
        try {
          ...
          const userInfoRes = await fetch(
            `/api/users/by-email?emails=${encodeURIComponent(emailsParam)}`
          );
          if (userInfoRes.ok) {
      ...
      const calculateDueDate = (ratingTimingId: number | null | undefined): string | null => {
        ...
        const stagesBeforeTarget = launchStages.filter(
          (stage) => stage.sort_order < targetStage.sort_order && stage.duration_days !== null
        );
      ...
      const resolvedMatrix = sorted.map((item: any) => {
        ...
        const calculatedDueDate =
          item.condition_due_date || calculateDueDate(item.criterion?.rating_timing);
      ...
      if (pod && settingsMapping[pod]) {
        ...
        const matchingKey = Object.keys(settingsMapping).find(
          (key) => key.toLowerCase() === podLower
        );
        if (matchingKey) {
      ...
      if (pmEmail) {
        ...
        try {
          const pmUserRes = await fetch(
            `/api/users/by-email?emails=${encodeURIComponent(normalizedEmail)}`
          );
          if (pmUserRes.ok) {
  ...
  async function handleTierUpdate(newTier: string | null) {
    ...
    if (!newTier || !epic || newTier === epic.tier) {
      console.log(
        'Early return: newTier=',
        newTier,
        'epic=',
        epic,
        'newTier === epic.tier',
        epic ? newTier === epic.tier : false
      );
      return;
    }
  ...
  async function retryInstantiate() {
    ...
    try {
      ...
      setInstantiationFailed(false);
      notifications.show({
        title: 'Criteria populated',
        message: 'Applicable criteria were added to this epic.',
        color: 'green',
      });
      await loadData();
    } catch (e: any) {
      notifications.show({
        title: 'Retry failed',
        message: e?.message || 'Could not populate criteria',
        color: 'red',
      });
    } finally {
    ...
    <div className="flex">
      <div className="flex-1 pt-24 pb-8 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex justify-between items-center">
          <Link href="/epics" className="text-blue-600 hover:text-blue-800 hover:underline">
            ← Back to Epics
          </Link>
          <Group gap="xs">
            {epic.aha_id && (
              <Button size="xs" variant="outline" onClick={handleSyncWithAha} loading={syncing}>
                Sync with Aha
              </Button>
            )}
            <Button size="xs" onClick={() => setSnapshotModalOpen(true)}>
              Take Snapshot
            </Button>
          </Group>
        </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2 items-center flex-wrap">
                {(() => {
                  ...
                  const pod =
                    (epic as any)?.pod || ahaFields?.custom_fields?.dev_backlog_pod || null;
                  return pod ? (
            ...
            <div className="grid grid-cols-4 gap-6">
              <div>
                ...
                <div className="text-2xl font-bold text-gray-900">
                  {matrix.length === 0
                    ? 'N/A'
                    : typeof epic.readiness_score === 'number'
              ...
              <div>
                ...
                <div className="text-sm font-semibold text-gray-900">
                  {matrix.length === 0 ? 'Not evaluated' : epic.readiness_status || 'Not set'}
                </div>
              </div>
              <div>
                <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">
        ...
        <div className="mb-8">
          ...
          {matrix.length === 0 ? (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-yellow-800 flex items-center justify-between gap-4">
              <div>
                No criteria configured. Add criteria in{' '}
                <Link
                  href="/admin/settings"
