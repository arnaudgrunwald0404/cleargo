  ...
  try {
    ...
    if (!process.env.MAGIC_LINK_SECRET) {
      ...
      return NextResponse.json(
        {
          error:
            'Magic link secret is not configured. Please set MAGIC_LINK_SECRET environment variable.',
        },
        { status: 500 }
      );
    }
    ...
    const adminClient = createAdminClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, secretKey);
    const { data: authUsers } = await adminClient.auth.admin.listUsers();
    const authUserMap = new Map(
      authUsers?.users.map((u) => [u.email?.toLowerCase(), !!u.last_sign_in_at]) || []
    );
    ...
    for (const targetUser of targetUsers) {
      try {
        const hasLoggedIn = authUserMap.get(targetUser.email?.toLowerCase() || '');
        ...
        const token = await createToken({ email: targetUser.email, jti, t: 'magic' }, '30m');
