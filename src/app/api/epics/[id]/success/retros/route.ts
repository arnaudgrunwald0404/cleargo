...
import { submitEpicRetroSchema } from '@/lib/success/validation';
import type { SubmitEpicRetroDTO } from '@/lib/success/types';
...
export async function GET(
  ...
  try {
    ...
    const retros = await getEpicRetros(epicId);
    return NextResponse.json(retros);
  } catch (error: any) {
    console.error('Error fetching epic retros:', error);
    return NextResponse.json(
      { error: 'Failed to fetch retros', details: error.message },
      { status: 500 }
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    const parsed = submitEpicRetroSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const submitData: SubmitEpicRetroDTO = {
      day_marker: parsed.data.day_marker,
    ...
    const retro = await submitEpicRetro(epicId, submitData.day_marker, submitData, me!.id);
    return NextResponse.json(retro, { status: 201 });
  } catch (error: any) {
    console.error('Error submitting epic retro:', error);
    return NextResponse.json(
      { error: 'Failed to submit retro', details: error.message },
      { status: 500 }
    );
  }
}
