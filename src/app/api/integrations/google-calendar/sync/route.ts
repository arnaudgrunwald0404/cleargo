    ...
    if (expiresAt <= new Date()) {
      ...
      if (!integration.refresh_token) {
        return NextResponse.json(
          {
            error:
              'Access token expired and no refresh token available. Please reconnect your Google Calendar in Account Details.',
            requiresReauth: true,
          },
          { status: 401 }
        );
      ...
      try {
        ...
        return NextResponse.json(
          {
            error:
              'Failed to refresh access token. Please reconnect your Google Calendar in Account Details.',
            requiresReauth: true,
          },
          { status: 401 }
        );
