    ...
    for (const ahaEpic of epics) {
      try {
        ...
        if (!force && !(await shouldProcessEpic(ahaEpic))) {
          console.log(
            `⏭️  Skipping epic ${ahaEpic.reference_num || ahaEpic.id}: Does not match filter criteria`
          );
          results.skipped++;
          continue;
        }
        ...
        try {
          ...
          console.warn(
            `Could not fetch full details for ${ahaEpic.reference_num}, using partial data`
          );
        }
