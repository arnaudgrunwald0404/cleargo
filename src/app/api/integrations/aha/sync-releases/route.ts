        ...
        for (const release of releasesToSync) {
            try {
                ...
                if (error) {
                    ...
                    // Cascade the updated date to all epics in this release
                    if (normalizedLaunchDate) {
                        try {
                            await cascadeReleaseDateToEpics(release.name, normalizedLaunchDate);
                        } catch (cascadeError) {
                            console.error(`Failed to cascade date for release "${release.name}":`, cascadeError);
                        }
                    }
                }
            } catch (error) {
