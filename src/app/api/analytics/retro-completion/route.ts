...
import { getRetroCompletionRate, getRetroCompletionRateTrends } from '@/lib/services/analyticsService';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (trends) {
      const result = await getRetroCompletionRateTrends(filters, monthsBack);
      return NextResponse.json(result);
    }

    const result = await getRetroCompletionRate(filters);
    return NextResponse.json(result);
  } catch (error: any) {
    console.error('Error fetching retro completion rate:', error);
    return NextResponse.json(
      { error: 'Failed to fetch retro completion rate', details: error.message },
      { status: 500 }
    );
  }
}
