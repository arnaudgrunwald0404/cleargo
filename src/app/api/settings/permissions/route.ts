...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to read permissions' },
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    if (body?.rules && typeof body.rules === 'object') {
      ...
      const entries = Object.entries(
        body.rules as Record<import('@/lib/permissions').CapabilityId, string[]>
      );
    ...
    if (body?.mapping && typeof body.mapping === 'object') {
      ...
      for (const [email, role] of Object.entries(body.mapping as Record<string, Role>)) {
        if (!ALL_ROLES.includes(role as Role)) {
          return NextResponse.json(
            { error: `Invalid role for ${email}: ${role}` },
            { status: 400 }
          );
        }
      }
    ...
    return NextResponse.json(
      { error: 'Invalid body: expected { rules } or { mapping }' },
      { status: 400 }
    );
  } catch (error: any) {
    return NextResponse.json(
