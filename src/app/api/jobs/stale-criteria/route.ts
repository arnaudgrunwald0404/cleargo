...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const criterion of activeCriteria) {
      ...
      const criterionData = Array.isArray(criterion.criterion)
        ? criterion.criterion[0]
        : criterion.criterion;
      const decisionOwner = Array.isArray(criterion.decision_owner)
        ? criterion.decision_owner[0]
        : criterion.decision_owner;
