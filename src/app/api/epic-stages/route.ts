...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01') {
        errorMessage =
          'The launch_stages table does not exist. Please run the migration: supabase/migrations/20251202000000_create_launch_stages.sql';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name || sort_order === undefined) {
      return NextResponse.json({ error: 'Name and sort_order are required' }, { status: 400 });
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('launch_stages').upsert(updates).select();
