...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!Array.isArray(epic_ids)) {
      return NextResponse.json({ error: 'epic_ids must be an array' }, { status: 400 });
    }

    // Check if meeting_epic table exists by trying to query it
    const { error: tableCheckError } = await supabase.from('meeting_epic').select('id').limit(1);

    if (
      tableCheckError &&
      (tableCheckError.message?.includes('relation') ||
        tableCheckError.code === '42P01' ||
        tableCheckError.code === 'PGRST')
    ) {
      ...
      if (epic_ids.length > 1) {
        return NextResponse.json(
          {
            error:
              'Multiple epic linking requires migration 0021. Please run the migration or link only one epic at a time.',
          },
          { status: 400 }
        );
      }
    ...
    if (epic_ids.length > 0) {
      ...
      const { error: insertError } = await supabase.from('meeting_epic').insert(links);
