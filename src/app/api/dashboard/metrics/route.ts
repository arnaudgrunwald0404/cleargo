  ...
  try {
    ...
    try {
      ...
      if (!activeError && activeCount !== null) {
        ...
        activeError &&
        (activeError.code === '42P01' || activeError.message?.includes('does not exist'))
      ) {
        // Try launch table
    ...
    try {
      ...
      if (!highRiskError && highRiskCount !== null) {
        ...
        highRiskError &&
        (highRiskError.code === '42P01' || highRiskError.message?.includes('does not exist'))
      ) {
        // Try launch table
    ...
    if (userEmail) {
      try {
        const { data: myItems, error: itemsError } = await supabase.rpc('my_items_for_user', {
          p_email: userEmail,
        });

        if (!itemsError && myItems) {
          // Count items that are NOT_SET or have status that indicates pending
          pendingItemsCount = myItems.filter(
            (item: any) =>
              item.status === 'NOT_SET' || item.status === 'CONDITIONAL' || !item.status
          ).length;
        }
      } catch (error) {
