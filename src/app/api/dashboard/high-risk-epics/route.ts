...
// Assess risk for a single epic
async function assessEpicRisk(epic: any, supabase: any): Promise<{ riskScore: number; riskReasons: string[] }> {
  ...
  if (readinessScore !== null && readinessScore !== undefined) {
    if (readinessScore < threshold) {
      ...
      riskScore += Math.min(20, Math.round(gap * 40));
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: epics, error: epicsError } = await supabase
      .from('epic')
      .select('id, name, tier, target_launch_date, readiness_score, readiness_status, risk_level, status')
    ...
    // Assess risk for each epic
    const riskAssessments = await Promise.all(
      epics.map(async (epic) => {
        const assessment = await assessEpicRisk(epic, supabase);
        return {
          ...epic,
    ...
    // Filter to high-risk epics (risk score >= 50) and sort by risk score
    const highRiskEpics = riskAssessments
      .filter((epic) => epic.riskScore >= 50)
      .sort((a, b) => b.riskScore - a.riskScore);

    return NextResponse.json({
      count: highRiskEpics.length,
      epics: highRiskEpics.map(epic => ({
        id: epic.id,
        name: epic.name,
        tier: epic.tier,
        ...
        riskScore: epic.riskScore,
        riskReasons: epic.riskReasons,
      })),
    });
  } catch (error: any) {
    console.error('Error identifying high-risk epics:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to identify high-risk epics' },
      { status: 500 }
    );
  }
}
