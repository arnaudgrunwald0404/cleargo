...
export default function Matrix({ epicId, epicName, items, onUpdate }: Props) {
  ...
  Object.keys(grouped).forEach((cat) => {
    ...
    const regularItems = items.filter(
      (item) => !item.criterion.label?.toLowerCase().startsWith('overall')
    );
  ...
  async function handleStatusChange(id: string, newStatus: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          responseData?.error || `Failed to update status: ${res.status} ${res.statusText}`
        );
      }
  ...
  return (
    <div className="space-y-8">
      {categories.map((cat) => {
        ...
        const allItems = overallItem ? [overallItem, ...regularItems] : regularItems;
              ...
              <table className="min-w-full divide-y divide-purple-200 table-fixed w-full">
                ...
                <thead className="bg-purple-100">
                  <tr>
                    ...
                    <th
                      className="px-4 py-2 text-left text-xs font-medium text-purple-900 normal-case"
                      style={{ width: '160px', textTransform: 'none' }}
                    >
                      Status
                    </th>
                    <th
                      className="px-4 py-2 text-left text-xs font-medium text-purple-900"
                      style={{ width: '200px' }}
                    >
                      Approver
                    </th>
                ...
                <tbody className="bg-white divide-y divide-purple-200">
                  {allItems.map((item, index) => {
                    ...
                    return (
                      <tr
                        key={item.id}
                        className={`hover:bg-purple-50 transition-colors ${isOverall ? 'cursor-pointer' : ''} ${item.notRequired ? 'opacity-60' : ''}`}
                        onClick={isOverall ? () => toggleCategory(cat) : undefined}
                      >
                        <td className="px-4 py-3">
                          <div
                            className={`font-medium flex items-center gap-2 ${item.notRequired ? 'text-gray-500' : 'text-gray-900'}`}
                          >
                            {isOverall && (
