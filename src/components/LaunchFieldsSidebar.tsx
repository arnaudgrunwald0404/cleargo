...
export default function LaunchFieldsSidebar({ launch }: LaunchFieldsSidebarProps) {
    const ahaFields = launch?.aha_fields || {};
    const standardFields = ahaFields.standard_fields || {};
    const customFields = ahaFields.custom_fields || {};
    
    // Extract writable fields from launch object (excluding hidden fields)
    ...
    const getFieldValue = (fieldKey: string): any => {
        // Check launch object first (for writable fields)
        if (launch && launch[fieldKey] !== null && launch[fieldKey] !== undefined) {
            return launch[fieldKey];
        }
    ...
    const formatReferenceNum = (): React.ReactNode => {
        ...
        const url = standardFields.url || launch?.aha_url;
