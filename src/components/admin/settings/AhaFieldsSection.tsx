      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className={`mb-6 p-4 rounded-lg border ${syncResult.failed > 0 ? 'bg-yellow-50 border-yellow-200 text-yellow-800' : 'bg-green-50 border-green-200 text-green-800'}`}>
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm mt-1">{syncResult.synced} succeeded, {syncResult.failed} failed out of {syncResult.total} total launches.</p>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Selected Fields ({settings.aha_fields_to_load?.length || 0})</h3>
                            ...
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  onChange={() => {
                                    const currentFields = settings.aha_fields_to_load || [];
                                    const newFields = currentFields.filter((f) => f !== field.alias);
                                    setSettings({ ...settings, aha_fields_to_load: newFields });
                                    onAutoSaveFields(newFields);
                                  }}
            ...
            <div>
              ...
              <div className="border-2 border-gray-200 rounded-lg bg-gray-50 overflow-hidden">
                {availableAhaFields.filter((f) => f.type !== "standard" && !settings.aha_fields_to_load?.includes(f.alias)).length > 0 ? (
                  ...
                  <div className="text-sm text-gray-400 italic text-center py-4">All fields are selected.</div>
                )}
              </div>
            </div>
          </div>
