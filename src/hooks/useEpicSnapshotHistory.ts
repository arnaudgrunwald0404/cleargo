  ...
  return useQuery({
    ...
    queryFn: async (): Promise<EpicSnapshotVersion[]> => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, created_at, snapshot_date, aha_key, aha_name, aha_status, aha_start_date, aha_end_date, aha_release, aha_pod, aha_owner, aha_t_shirt_est, aha_csm_priority, aha_progress, jira_key',
        )
        .eq('aha_key', ahaKey)
