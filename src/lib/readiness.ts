...
export async function recomputeEpicReadiness(epicId: string) {
  ...
  const { data: epic, error: epicError } = await supabase
    .from('epic')
    .select(
      'id, name, tier, target_launch_date, readiness_status, risk_level, console_url, owner_email'
    )
    .eq('id', epicId)
    .single();
  ...
  if (gateNoGoCount > 0) {
    ...
    if (readinessScore >= threshold) {
      ...
      // Arbitrary "close enough" for Conditional?
