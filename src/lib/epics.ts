...
export async function instantiateEpicMatrix(epicId: string, tier: string) {
  ...
  // 4. Insert rows
  const { error: insertError } = await supabase.from('epic_criterion_status').insert(rows);
    ...
    if (error) {
      ...
      if (isJWTError) {
        ...
        if (usingNewFormat && hasLegacyKeys) {
          ...
          console.warn(
            '   To avoid this warning, use legacy keys: SUPABASE_SERVICE_ROLE_KEY and NEXT_PUBLIC_SUPABASE_ANON_KEY'
          );
          ...
          try {
            ...
            if (
              retryResult.error &&
              (retryResult.error.code === '42P01' ||
                retryResult.error.message?.includes('does not exist'))
            ) {
        ...
        if (usingNewFormat) {
          ...
          console.error('');
          console.error(
            "   ⚠️  SOLUTION: PostgREST (database queries) doesn't support new format keys yet."
          );
          ...
          if (hasLegacyKeys) {
            ...
            console.error(
              '   1. Verify keys in Supabase Dashboard → Settings → API → "Legacy anon, service_role API keys"'
            );
            ...
            console.error(
              '   Please set SUPABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_SUPABASE_ANON_KEY'
            );
          }
          return [];
        }
...
export async function deleteEpic(id: string) {
  ...
  const { error } = await supabase.from('epic').delete().eq('id', id);
