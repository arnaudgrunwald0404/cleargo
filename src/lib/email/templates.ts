...
/**
 * Master Approval Ready email (CLEARGO-I-9) — final approver, all department
 * gates signed off.
 */
export function getMasterApprovalReadyEmail(params: {
    recipientName: string;
    epicName: string;
    epicId: string;
    gateCount: number;
    ...
    const epicUrl = `${appBaseUrl}/epics/${epicId}`;
    const greeting = recipientName ? `Hi ${recipientName},` : 'Hello,';
    const subject = `[ClearGO] Ready for final approval — ${epicName}`;
    ...
    const html = `
        <div style="font-family: 'Public Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p style="color: #374151; font-size: 15px; line-height: 1.6; margin: 0 0 20px;">
                All <strong>${gateCount} department gate${gateCount === 1 ? '' : 's'}</strong> for <strong>${epicName}</strong>
                have been signed off. It's ready for your final Go/No-Go decision.
            </p>
            ...
            <p style="color: #9ca3af; font-size: 13px; line-height: 1.6; border-top: 1px solid #f3f4f6; padding-top: 16px; margin: 0;">
                You received this because you are a final approver for <em>${epicName}</em>.
                Manage your notification preferences in
                <a href="${appBaseUrl}/settings" style="color: #4f46e5; text-decoration: none;">My Settings</a>.
            </p>
        </div>
