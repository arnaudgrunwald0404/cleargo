...
export async function getSettings(): Promise<AppSettings> {
  ...
  debugLog({
    location: 'settings-db.ts:getSettings',
    message: 'Fetched settings from DB',
    data: {
      hasData: !!data,
      ahaFieldsFromDB: data?.aha_fields_to_load,
      hasDuplicates: data?.aha_fields_to_load
        ? new Set(data.aha_fields_to_load).size !== data.aha_fields_to_load.length
        : false,
    },
    hypothesisId: 'C',
  });
  ...
  if (
    ...
    (Array.isArray(data.aha_fields_to_load) && data.aha_fields_to_load.length === 0)
  ) {
    data.aha_fields_to_load = [
      'dev_backlog_pod',
      'primary_goal',
...
export async function updateSettings(
  ...
  const supabase = await createClient();
  debugLog({
    location: 'settings-db.ts:updateSettings-START',
    message: 'updateSettings called',
    data: {
      updateKeys: Object.keys(updates),
      ahaFieldsInUpdate: updates.aha_fields_to_load,
      hasDuplicatesInUpdate: updates.aha_fields_to_load
        ? new Set(updates.aha_fields_to_load).size !== updates.aha_fields_to_load.length
        : false,
    },
    hypothesisId: 'A',
  });
  ...
  debugLog({
    ...
    data: {
      mergedAhaFields: updateData.aha_fields_to_load,
      hasDuplicatesAfterMerge: updateData.aha_fields_to_load
        ? new Set(updateData.aha_fields_to_load).size !== updateData.aha_fields_to_load.length
        : false,
    },
    hypothesisId: 'A',
  });
  ...
  const { data, error } = await supabase
    ...
    .maybeSingle();
  debugLog({
    location: 'settings-db.ts:updateSettings-RESPONSE',
    message: 'DB upsert response',
    data: {
      hasData: !!data,
      hasError: !!error,
