      ...
      for (const c of record.claims) {
        if (c && typeof c === 'object' && 'text' in c) {
          results.push({
            ...
            claim: {
              text: String(c.text ?? ''),
              source: String((c as any).source ?? 'unknown'),
              grounded: Boolean((c as any).grounded),
            },
          });
        }
      }
      ...
      for (const c of record.checks) {
        if (c && typeof c === 'object' && 'check' in c) {
          results.push({
            ...
            claim: {
              text: String(c.check ?? ''),
              source: String((c as any).evidence ?? ''),
              grounded: (c as any).verdict !== 'unknown',
            },
          });
        }
      }
...
export async function explainClaim(args: Record<string, unknown>): Promise<unknown> {
