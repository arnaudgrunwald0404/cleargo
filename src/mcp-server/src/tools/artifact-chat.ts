...
function analyzeSections(aiDraft: Record<string, unknown>, artifactType: string): Array<{
  section: string;
  present: boolean;
  hasContent: boolean;
  hasClaims: boolean;
  ...
  for (const [key, value] of Object.entries(aiDraft)) {
    ...
    if (isObject && Array.isArray((value as any).claims)) {
      const claims = (value as any).claims;
      claimCount = claims.length;
      groundedClaims = claims.filter((c: any) => c.grounded === true).length;
    }
...
function countFlags(flags: Array<any> = []): { open: number; asked: number; total: number } {
...
export async function artifactChat(args: Record<string, unknown>): Promise<unknown> {
  ...
  const sections = aiDraft ? analyzeSections(aiDraft, parsed.data.artifactType) : [];
