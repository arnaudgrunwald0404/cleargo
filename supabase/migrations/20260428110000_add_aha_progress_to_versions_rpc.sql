-- Re-add `aha_progress` to get_latest_and_previous_roadmap_versions().
--
-- The seed migration (20260427120000_roadmap_rewind_functions.sql) had two
-- versions of this function defined in sequence — the *later* one (added
-- to bring `aha_csm_priority` in) accidentally dropped `aha_progress`.
-- We need it back so the snapshot table can render the inline progress
-- bar that RRV's UI shows under each item title.
