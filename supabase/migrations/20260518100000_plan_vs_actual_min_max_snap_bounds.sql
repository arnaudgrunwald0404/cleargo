-- Plan vs Actual: `quarter_snap_dates` / `month_snap_dates` scanned every Cleargo snapshot row in the
-- quarter/month only to compute MIN/MAX snapshot_date. Replace with three scalar aggregates (same bounds,
-- far less I/O). Keeps behavior from 20260517100000_optimize_get_period_plan_vs_actual.sql.
...
COMMENT ON FUNCTION public.get_period_plan_vs_actual(text, date) IS
  'Plan vs Actual: ClearGO candidate epics only. Snap bounds use MIN/MAX aggregates (no DISTINCT date scans). statement_timeout 120s.';
