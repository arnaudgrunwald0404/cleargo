...
-- Restrict write policies so only users that exist in app_user can INSERT/UPDATE/DELETE.
-- Expression: EXISTS (SELECT 1 FROM public.app_user WHERE LOWER(email) = LOWER(auth.jwt()->>'email'))

-- Index to speed RLS checks that use LOWER(email) = LOWER(auth.jwt()->>'email')
CREATE INDEX IF NOT EXISTS idx_app_user_lower_email ON public.app_user (LOWER(email));
...
CREATE POLICY "Authenticated users can update app_settings" ON public.app_settings
  ...
  USING (EXISTS (SELECT 1 FROM public.app_user WHERE LOWER(email) = LOWER(auth.jwt()->>'email')))
  WITH CHECK (EXISTS (SELECT 1 FROM public.app_user WHERE LOWER(email) = LOWER(auth.jwt()->>'email')));
