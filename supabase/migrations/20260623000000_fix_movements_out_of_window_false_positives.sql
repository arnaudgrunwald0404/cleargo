-- Fix false positives in get_year_movements_with_impact for items that
-- disappear from the Aha pivot but haven't actually changed release.
--
-- The pivot only shows ~5 upcoming releases. When an item falls off the pivot
-- edge (or is temporarily absent), the old logic reports to_release = NULL
-- ("Moved 3+ releases out"). This was causing false alerts in the weekly
-- Slack notification.
--
-- Fix: LEFT JOIN the epic table to cross-reference the current Aha release.
--   - If epic still has the SAME release as from_release → suppress (false positive)
--   - If epic has a DIFFERENT release → use it as to_release (real movement beyond window)
--   - If epic is archived or not in ClearGO → keep NULL (genuinely removed)
...
BEGIN
  ...
  -- Normal movements: item is in BOTH snapshots but release changed
  movements AS (
    SELECT
      sp.curr_snap_date,
  ...
  -- Out-of-window movements: item was in prev snapshot but not in current.
  -- Cross-reference the epic table to distinguish real movements from false positives.
  movements_out_of_window AS (
    SELECT
      ...
      -- If the epic table has a different release, use it; otherwise NULL
      CASE
        WHEN e.aha_id IS NOT NULL
          AND NOT COALESCE(e.archived, false)
          AND NULLIF(TRIM(COALESCE(
            e.aha_fields->'standard_fields'->>'aha_release_name',
            e.aha_fields->'standard_fields'->'release'->>'name',
            ''
          )), '') IS NOT NULL
          AND TRIM(COALESCE(
            e.aha_fields->'standard_fields'->>'aha_release_name',
            e.aha_fields->'standard_fields'->'release'->>'name',
            ''
          )) != TRIM(prev.aha_release)
        THEN TRIM(COALESCE(
          e.aha_fields->'standard_fields'->>'aha_release_name',
          e.aha_fields->'standard_fields'->'release'->>'name'
        ))
        ELSE NULL
      END as current_release,
    ...
    LEFT JOIN public.epic e ON e.aha_id = prev.aha_key
    WHERE sp.previous_date IS NOT NULL
      ...
      -- Suppress false positives: if the epic table shows the SAME release
      -- as from_release, the item didn't actually move — it just fell off the pivot
      AND NOT (
        e.aha_id IS NOT NULL
        AND NOT COALESCE(e.archived, false)
        AND TRIM(COALESCE(
          e.aha_fields->'standard_fields'->>'aha_release_name',
          e.aha_fields->'standard_fields'->'release'->>'name',
