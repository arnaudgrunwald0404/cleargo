-- Speed up Plan vs Actual RPC: remove per-row correlated EXISTS, dedupe quarter/month date scans,
-- and raise statement_timeout for this function only (pool still caps global max).
...
quarter_snap_dates AS (
  SELECT DISTINCT rs.snapshot_date AS d
  FROM public.roadmap_snapshot rs
  INNER JOIN cleargo_aha c ON c.aha_key = rs.aha_key
  INNER JOIN params p ON rs.snapshot_date BETWEEN p.quarter_start AND p.quarter_end
),
month_snap_dates AS (
  SELECT DISTINCT rs.snapshot_date AS d
  FROM public.roadmap_snapshot rs
  INNER JOIN cleargo_aha c ON c.aha_key = rs.aha_key
  INNER JOIN params p ON p.mode = 'quarter_progress'
    AND rs.snapshot_date BETWEEN p.month_start AND p.month_end
),
snap_bounds AS (
  SELECT
    p.mode,
    ...
    p.month_start,
    p.month_end,
    (SELECT min(d) FROM quarter_snap_dates) AS start_dt,
    CASE p.mode
      WHEN 'quarter_baseline' THEN (SELECT min(d) FROM quarter_snap_dates)
      WHEN 'quarterly' THEN (SELECT max(d) FROM quarter_snap_dates)
      WHEN 'quarter_progress' THEN (SELECT max(d) FROM month_snap_dates)
    END AS end_dt
  FROM params p
...
COMMENT ON FUNCTION public.get_period_plan_vs_actual(text, date) IS
  'Plan vs Actual: ClearGO candidate epics only. Optimized: single pass for quarter snapshot dates, no per-row EXISTS for in_end; statement_timeout 120s for this function.';
